...
## Description: This script executes a grid search over the parameter space defined in the dissertation to find the best XGBoost (XGB) model according to the MCC variable. 
...
#Creating a base XGBoost model, the parameters are supplied later during the grid search procedure.
model = XGBClassifier()
...
dump(grid_imba.best_estimator_, 'trained_models\\xgb_model.joblib') 
dump(grid_imba.cv_results_, 'trained_models\\xgb_training_results.joblib') 
